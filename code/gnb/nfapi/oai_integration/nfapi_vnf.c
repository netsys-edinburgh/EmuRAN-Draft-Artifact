...
int pnf_nr_param_resp_cb(nfapi_vnf_config_t *config, int p5_idx, nfapi_nr_pnf_param_response_t *resp) {
  ...
  for(int i = 0; i < resp->pnf_phy.number_of_phys; ++i) {
    ...
    phy.id = vnf->p7_vnfs[0].config->phy_id;
    NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] (PHY:%d) phy_config_idx:%d phy.id:%d\n ", i, resp->pnf_phy.phy[i].phy_config_index, phy.id);
    nfapi_vnf_allocate_phy(config, p5_idx, &(phy.id));
    NFAPI_TRACE(NFAPI_TRACE_INFO, "[VNF] (PHY:%d) phy.id:%d\n ", i, phy.id);
...
void configure_nr_nfapi_vnf(char *vnf_addr, int vnf_p5_port, char *pnf_ip_addr, int pnf_p7_port, int vnf_p7_port, int *gnb_id)  {
  ...
  vnf.p7_vnfs[0].config->phy_id = id;
...
int oai_nfapi_dl_config_req(nfapi_dl_config_request_t *dl_config_req) {
  ...
  dl_config_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id;; // HACK TODO FIXME - need to pass this around!!!!
...
{
  LOG_D(NR_PHY, "[phy_id=%d]Entering oai_nfapi_nr_tx_data_req sfn:%d,slot:%d\n", vnf.p7_vnfs[0].config->phy_id,  tx_data_req->header.phy_id, tx_data_req->SFN, tx_data_req->Slot);
  nfapi_vnf_p7_config_t *p7_config = vnf.p7_vnfs[0].config;
  tx_data_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id; // HACK TODO FIXME - need to pass this around!!!!
...
{
  nfapi_vnf_p7_config_t *p7_config = vnf.p7_vnfs[0].config;
  tx_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id; // HACK TODO FIXME - need to pass this around!!!!
...
int oai_nfapi_ul_dci_req(nfapi_nr_ul_dci_request_t *ul_dci_req) {
  nfapi_vnf_p7_config_t *p7_config = vnf.p7_vnfs[0].config;
  ul_dci_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id;; // HACK TODO FIXME - need to pass this around!!!!
...
int oai_nfapi_hi_dci0_req(nfapi_hi_dci0_request_t *hi_dci0_req) {
  nfapi_vnf_p7_config_t *p7_config = vnf.p7_vnfs[0].config;
  hi_dci0_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id;; // HACK TODO FIXME - need to pass this around!!!!
...
int oai_nfapi_ul_tti_req(nfapi_nr_ul_tti_request_t *ul_tti_req) {
  ...
  ul_tti_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id; // HACK TODO FIXME - need to pass this around!!!!
...
int nfapi_vnf_p7_nr_slot_ind(nfapi_vnf_p7_config_t* config, nfapi_nr_slot_indication_scf_t* ind)
{
	...
	return vnf_nr_p7_pack_and_send_p7_msg(vnf_p7, &ind->header);
}

int oai_nfapi_vnf_nr_slot_indication(nfapi_nr_slot_indication_scf_t *ind) {
  nfapi_vnf_p7_config_t *p7_config_g = vnf.p7_vnfs[0].config;
  ind->header.phy_id = vnf.p7_vnfs[0].config->phy_id; // HACK TODO FIXME - need to pass this around
  return nfapi_vnf_p7_nr_slot_ind(p7_config_g, ind);
}
...
int oai_nfapi_ue_release_req(nfapi_ue_release_request_t *release_req){
    ...
    release_req->header.phy_id = vnf.p7_vnfs[0].config->phy_id; // HACK TODO FIXME - need to pass this around!!!!
