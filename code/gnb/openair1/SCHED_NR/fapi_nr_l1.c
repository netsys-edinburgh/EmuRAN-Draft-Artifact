...
*/
  ...
  // send subframe indication to PNF
    ...
    slot_ind = (nfapi_nr_slot_indication_scf_t *)malloc(sizeof(nfapi_nr_slot_indication_scf_t));
    memset(slot_ind,0,sizeof(nfapi_nr_slot_indication_scf_t));
    slot_ind->header.message_id = NFAPI_NR_PHY_MSG_TYPE_SLOT_INDICATION;
    slot_ind->sfn= frame;
    slot_ind->slot= slot;
    oai_nfapi_vnf_nr_slot_indication(slot_ind);
