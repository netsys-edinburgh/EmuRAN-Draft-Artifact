...
int pnf_connect(pnf_t* pnf)
{
	...
	while(p != NULL && connected == 0)
	{
		...
		// bind P5 to the correct IP
		nfapi_pnf_config_t* config = (nfapi_pnf_config_t* ) pnf;
		struct sockaddr_in address;
		memset(&address, 0, sizeof(address));
		pnf_info * pnf_inf = (pnf_info *) config->user_data;
		address.sin_addr.s_addr = inet_addr(pnf_inf->phys[0].local_addr);
		address.sin_family = AF_INET;
		address.sin_port = htons(0);
		NFAPI_TRACE(NFAPI_TRACE_INFO, "Binding P5 socket to %s (random port)\n", pnf_inf->phys[0].local_addr);
		if (bind(pnf->p5_sock, (struct sockaddr *) &address, sizeof(address)) < 0)
		{
			NFAPI_TRACE(NFAPI_TRACE_ERROR, "Failed to bind() to address\n");
			return -1;
		}
...
int pnf_message_pump(pnf_t* pnf)
{
	...
	while(socketConnected && pnf->terminate == 0)
	{
		...
		{
			NFAPI_TRACE(NFAPI_TRACE_WARN, "P5 poll() failed\n");
			sleep(1);
			continue;
		}
		...
		{
			NFAPI_TRACE(NFAPI_TRACE_WARN, "P5 socket error revents=0x%x, disconnecting\n", pfd.revents);
			socketConnected = 0;
		}
