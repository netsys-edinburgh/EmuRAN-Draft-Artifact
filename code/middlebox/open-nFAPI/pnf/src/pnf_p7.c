...
nfapi_nr_slot_indication_scf_t* allocate_nfapi_slot_indication(pnf_p7_t* pnf_p7) 
{ 
	void *ptr= pnf_p7_malloc(pnf_p7, sizeof(nfapi_nr_slot_indication_scf_t));
		//printf("%s() ptr:%p\n", __FUNCTION__, ptr);
	return ptr;
}
...
void pnf_handle_dl_tti_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
	...
	// printf("[%d.%d] dl tti received \n", req->SFN, req->Slot);
	if(unpack_result == 0)
	{
                NFAPI_TRACE(NFAPI_TRACE_INFO, "POPULATE DL_TTI_REQ current tx sfn/slot:%d.%d\n",
                                              req->SFN, req->Slot);
                transfer_downstream_nfapi_msg_to_nr_proxy(pnf_p7->_public.pnf_id, (void *)req);
	}
	else
	{
...
void pnf_handle_slot_indication(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
	nfapi_nr_slot_indication_scf_t* req  = allocate_nfapi_slot_indication(pnf_p7);
    
    
    
	int unpack_result = nfapi_nr_p7_message_unpack(pRecvMsg, recvMsgLen, req, sizeof(nfapi_nr_slot_indication_scf_t), &(pnf_p7->_public.codec_config));
	// printf("[%d.%d] slot ind received \n", req->sfn, req->slot);
	if(unpack_result == 0){
		if(pthread_mutex_lock(&(pnf_p7->mutex)) != 0)
		{
			NFAPI_TRACE(NFAPI_TRACE_ERROR, "failed to lock mutex\n");
...
void pnf_handle_ul_tti_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
	...
	if(unpack_result == 0)
	{
		...
		{   
			// printf("[%d.%d] ul tti request is out of window \n", req->SFN, req->Slot);
...
void pnf_handle_tx_data_request(void* pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7)
{
	...
	// printf("[%d.%d] tx data request received \n", req->SFN, req->Slot);
	if(unpack_result == 0)
	{
...
void pnf_nr_dispatch_p7_message(void *pRecvMsg, int recvMsgLen, pnf_p7_t* pnf_p7,  uint32_t rx_hr_time)
{
	...
	switch (header.message_id)
	{
		...
		case NFAPI_NR_PHY_MSG_TYPE_SLOT_INDICATION:
			pnf_handle_slot_indication(pRecvMsg, recvMsgLen, pnf_p7);
			break;
...
*/
	...
	{
		addr.sin_addr.s_addr = proxy_ip_addr;
	}
	else
	{
