...
uint8_t enb_ue_count[MAX_ENB+MAX_UES]; 
extern int queue_type; 
#define MAX_UNIQUE_IDS 8
...
nfapi_pnf_p7_config_t *p7_nr_config_g[MAX_GNB];
...
static pnf_info pnf;
static pnf_info pnf_nr[MAX_GNB];
static pthread_t pnf_start_pthread;
static pthread_t pnf_nr_start_pthread[MAX_GNB];
...
void *pnf_nr_p7_thread_start(void *ptr) {
  ...
  pnf_set_thread_priority(89);
  nfapi_pnf_p7_config_t *config = (nfapi_pnf_p7_config_t *)ptr;
   NFAPI_TRACE(NFAPI_TRACE_INFO, "[PNF %d] P7 THREAD %s\n", config->phy_id, __FUNCTION__);
...
*/
  ...
  nfapi_resp.nfapi_config.p7_pnf_port.value = 50610 + (8*req->header.phy_id); //pnf->phys[0].local_port; DJP - hard code alert!!!! FIXME TODO
  nfapi_resp.num_tlv++;
  pnf_p7_sockaddr.sin_addr.s_addr = inet_addr(pnf->phys[0].local_addr);
  memcpy(nfapi_resp.nfapi_config.p7_pnf_address_ipv4.address, &pnf_p7_sockaddr.sin_addr.s_addr, 4);
  nfapi_resp.num_tlv++;
...
int nr_config_request(nfapi_pnf_config_t *config, nfapi_pnf_phy_config_t *phy, nfapi_nr_config_request_scf_t *req)
{
  ...
  if(req->carrier_config.dl_bandwidth.tl.tag == NFAPI_NR_CONFIG_DL_BANDWIDTH_TAG) {
    ...
    printf("DL Bandwidth: %d\n", phy_info->dl_channel_bw_support);
    num_tlv++;
  } else {            
  }
...
int nr_start_request(nfapi_pnf_config_t *config, nfapi_pnf_phy_config_t *phy,  nfapi_nr_start_request_scf_t *req) {
  ...
  */
  ...
  NFAPI_TRACE(NFAPI_TRACE_INFO, "[PNF %d] DJP - HACK - Set p7_config global ready for subframe ind\n", p7_config->pnf_id);
  printf("p7_config_g pnf id %d", p7_config->pnf_id);
  p7_nr_config_g[p7_config->pnf_id] = p7_config;
  ...
  */
  ...
  NFAPI_TRACE(NFAPI_TRACE_DEBUG, "[PNF %d] Sending first P7 slot indication\n", p7_config->phy_id);
...
void configure_nr_nfapi_pnf(int id, const char *vnf_ip_addr, int vnf_p5_port, const char *pnf_ip_addr, int pnf_p7_port, int vnf_p7_port) {
  nfapi_pnf_config_t *config = nfapi_pnf_config_create();
  config->vnf_ip_addr = vnf_ip_addr;
  config->vnf_p5_port = vnf_p5_port;
  pnf_nr[id].phys[0].udp.enabled = 1;
  pnf_nr[id].phys[0].udp.rx_port = pnf_p7_port;
  pnf_nr[id].phys[0].udp.tx_port = vnf_p7_port;
  strcpy(pnf_nr[id].phys[0].udp.tx_addr, vnf_ip_addr);
  strcpy(pnf_nr[id].phys[0].local_addr, pnf_ip_addr);
  NFAPI_TRACE(NFAPI_TRACE_DEBUG,
         "%s() VNF:%s:%d PNF_PHY[addr:%s UDP:tx_addr:%s:%d rx:%d]\n",
         __FUNCTION__,config->vnf_ip_addr, config->vnf_p5_port,
         pnf_nr[id].phys[0].local_addr,
         pnf_nr[id].phys[0].udp.tx_addr, pnf_nr[id].phys[0].udp.tx_port,
         pnf_nr[id].phys[0].udp.rx_port);
  ...
  pthread_create(&pnf_nr_start_pthread[id], NULL, &pnf_nr_start_thread, config);
  pthread_setname_np(pnf_nr_start_pthread[id], "NFAPI_PNF");
...
void oai_slot_ind(int id, uint16_t sfn, uint16_t slot)
{
    //TODO FIXME - HACK - DJP - using a global to bodge it in
    if (p7_nr_config_g[id] != NULL)
    {
        ...
        ind.sfn = sfn;
        ind.slot = slot;
        ind.header.phy_id = p7_nr_config_g[id]->phy_id;
        ind.header.message_id = NFAPI_NR_PHY_MSG_TYPE_SLOT_INDICATION;
        int slot_ret = nfapi_pnf_p7_nr_slot_ind(p7_nr_config_g[id], &ind);
        ...
        {
            NFAPI_TRACE(NFAPI_TRACE_DEBUG, "[PNF: %d] Sent slot indication from PNF to VNF in oai_slot_ind %4d.%2d\n", 
                        id ,sfn, slot);
        }
    }
    else
...
static queue_t msgs_from_nr_ue[MAX_GNB][MAX_UES];
...
void oai_slot_init()
{  for(int j = 0; j < num_gnb; j++){
    for (int i = 0; i < num_ues; i++)
    {
        init_queue(&msgs_from_nr_ue[j][i]);
    }
}
}
...
static void oai_slot_aggregate_rach_ind(int id,slot_msgs_t *msgs)
{
    /* Currently we select the first RACH and ignore others.
       We could have various policies to select one RACH over
       the others (e.g., strongest signal) */
    ...
    for (int i = 0; i < msgs->num_msgs; ++i)
    {
        ...
        oai_nfapi_nr_rach_indication(id, &ind);
    }
}
...
static void oai_slot_aggregate_crc_ind(int id, slot_msgs_t *msgs)
{
    assert(msgs->num_msgs > 0);
    ...
    oai_nfapi_nr_crc_indication(id, &agg);
    free(agg.crc_list);
}
...
static void oai_slot_aggregate_rx_data_ind(int id, slot_msgs_t *msgs)
{
    ...
    oai_nfapi_nr_rx_data_indication(id, &agg);
    free(agg.pdu_list);
}
...
static void oai_slot_aggregate_uci_ind(int id, slot_msgs_t *msgs)
{
    assert(msgs->num_msgs > 0);
    ...
    oai_nfapi_nr_uci_indication(id, &agg);
...
static void oai_slot_aggregate_srs_ind(int id, slot_msgs_t *msgs)
{
    assert(msgs->num_msgs > 0);
    ...
    oai_nfapi_nr_srs_indication(id, &agg);
...
static void oai_slot_aggregate_message_id(int id, uint16_t msg_id, slot_msgs_t *msgs)
{
    assert(msgs->num_msgs > 0);
    assert(msgs->msgs[0] != NULL);
    assert(msgs->msgs[0]->length <= sizeof(msgs->msgs[0]->data));
    ...
    {
    case NFAPI_NR_PHY_MSG_TYPE_RACH_INDICATION:
        oai_slot_aggregate_rach_ind(id, msgs);//It is using subframe. Need to check further.
        break;
    case NFAPI_NR_PHY_MSG_TYPE_CRC_INDICATION:
        oai_slot_aggregate_crc_ind(id, msgs);
        break;
    case NFAPI_NR_PHY_MSG_TYPE_RX_DATA_INDICATION:
        oai_slot_aggregate_rx_data_ind(id, msgs);
        break;
    case NFAPI_NR_PHY_MSG_TYPE_UCI_INDICATION:
        oai_slot_aggregate_uci_ind(id,msgs);
        break;
    case NFAPI_NR_PHY_MSG_TYPE_SRS_INDICATION:
        oai_slot_aggregate_srs_ind(id, msgs);
        break;
    default:
        return;
    }
...
*/
static void oai_slot_aggregate_messages(int id, slot_msgs_t *slot_msgs)
{
    for (;;)
    {
        ...
        oai_slot_aggregate_message_id(id, found_msg_id, &msgs_by_id);
...
bool dequeue_ue_slot_msgs(int phy_id, slot_msgs_t *slot_msgs, uint16_t sfn_slot_tx)
{
    // Dequeue for all UE responses, and discard any with the wrong sfn/sf value.
    // There might be multiple messages from a given UE with the same sfn/sf value.
    bool are_queues_empty = true;
    ...
    for (int i = 0; i < num_ues; i++)
    {
        for (;;)
        {
            message_buffer_t *msg = get_queue(&msgs_from_nr_ue[phy_id][i]);
            if (!msg)
            {
                break;
            }
            ...
            {
                if (!requeue(&msgs_from_nr_ue[phy_id][i], msg))
                {
                    msg->magic = 0;
                    free(msg);
                }
...
void *oai_slot_task(void *context)
{
    ...
    printf("oai_slot_task: softmodem_mode %d\n", softmodem_mode);
    NFAPI_TRACE(NFAPI_TRACE_INFO, "Slot Task thread");
    //uint16_t slot_tick = 0;
    int id = ((struct oai_task_args*)context)->node_id;
    int num_gnbs = ((struct oai_task_args *)context)->num_enbs;
    free(context);
    printf("Slot Task thread for node %d, num_gnbs %d\n", id, num_gnbs);
    int skip_first = 0;
    // uint16_t last_sfn_slot_tx = 0;
    while (true)
    {
        ...
        */
        ...
        are_queues_empty = dequeue_ue_slot_msgs(id, slot_msgs, sfn_slot_tx);
        oai_slot_aggregate_messages(id, slot_msgs);
        usleep(200);
        oai_slot_aggregate_messages(id, slot_msgs);
        if (skip_first)
            skip_first = 0;
        else
            oai_slot_ind(id, NFAPI_SFNSLOT2SFN(sfn_slot_tx), NFAPI_SFNSLOT2SLOT(sfn_slot_tx));
        // last_sfn_slot_tx = sfn_slot_tx;
        //usleep(100);
        if (softmodem_mode == SOFTMODEM_NR_N_GNB)
        {
            ...
            pnfs_done_sf++;
            if (pnfs_done_sf == num_gnbs)
            {
                // printf("eNB[%d] pnfs_done_sf %d\n", id, pnfs_done_sf);
        ...
        if(softmodem_mode == CHRONOS_5G){
            // printf("oai_slot_task: id %d, sfn %d, slot %d\n", id, sfn, slot);
            if (pthread_mutex_lock(&lock) != 0)
            {
                errExit("failed to lock mutex");
            }
            pnfs_done_sf++;
            // printf("pnfs_done_sf = %d, num_gnbs = %d\n", pnfs_done_sf, num_gnbs);
            if (pnfs_done_sf == num_gnbs)
            {
                while (1) {
                if (g_all_acks_received) {
                    // printf("All acks received, resetting g_all_acks_received\n");
        ...
        // printf("[%d.%d] Slot Finished \n", NFAPI_SFNSLOT2SFN(sfn_slot_tx), NFAPI_SFNSLOT2SLOT(sfn_slot_tx));
    }
}
...
void oai_slot_handle_msg_from_ue(int phy_id, const void *msg, size_t len, uint16_t nem_id)
{
    if (len == 4) // Dummy packet ignore
    {
        return;
    ...
    if (!put_queue(&msgs_from_nr_ue[phy_id][i], p))
    {
        p->magic = 0;
        free(p);
    }
...
int oai_nfapi_nr_rx_data_indication(int id, nfapi_nr_rx_data_indication_t *ind) {
  ind->header.phy_id = id; // DJP HACK TODO FIXME - need to pass this around!!!!
  ind->header.message_id = NFAPI_NR_PHY_MSG_TYPE_RX_DATA_INDICATION;
  return nfapi_pnf_p7_nr_rx_data_ind(p7_nr_config_g[id], ind);
}

int oai_nfapi_nr_crc_indication(int id, nfapi_nr_crc_indication_t *ind) {
  ind->header.phy_id = id; // DJP HACK TODO FIXME - need to pass this around!!!!
  ind->header.message_id = NFAPI_NR_PHY_MSG_TYPE_CRC_INDICATION;
  return nfapi_pnf_p7_nr_crc_ind(p7_nr_config_g[id], ind);
}

int oai_nfapi_nr_srs_indication(int id, nfapi_nr_srs_indication_t *ind) {
  ind->header.phy_id = id; // DJP HACK TODO FIXME - need to pass this around!!!!
  ind->header.message_id = NFAPI_NR_PHY_MSG_TYPE_SRS_INDICATION;
  return nfapi_pnf_p7_nr_srs_ind(p7_nr_config_g[id], ind);
}

int oai_nfapi_nr_uci_indication(int id, nfapi_nr_uci_indication_t *ind) {
  ind->header.phy_id = id; // DJP HACK TODO FIXME - need to pass this around!!!!
  ind->header.message_id = NFAPI_NR_PHY_MSG_TYPE_UCI_INDICATION;
  return nfapi_pnf_p7_nr_uci_ind(p7_nr_config_g[id], ind);
}

int oai_nfapi_nr_rach_indication(int id, nfapi_nr_rach_indication_t *ind) {
  ind->header.phy_id = id; // DJP HACK TODO FIXME - need to pass this around!!!!
  ind->header.message_id = NFAPI_NR_PHY_MSG_TYPE_RACH_INDICATION;
  return nfapi_pnf_p7_nr_rach_ind(p7_nr_config_g[id], ind);
}
