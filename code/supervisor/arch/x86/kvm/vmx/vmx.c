...
#endif
extern atomic64_t global_tsc;
static atomic64_t *global_tsc_ptr = NULL;
...
{   static u64 rdtsc_real;
    if (!global_tsc_ptr) {
        rdtsc_real = rdtsc();
    }
	else{
		rdtsc_real = atomic64_read(global_tsc_ptr);
	}
	
    vcpu->arch.regs[VCPU_REGS_RAX] = rdtsc_real & -1u;
    vcpu->arch.regs[VCPU_REGS_RDX] = (rdtsc_real >> 32) & -1u;
...
{
	int r, cpu;
    global_tsc_ptr = symbol_get(global_tsc);
