...
\title{Artifact: EmuRAN: Scalable and Cost-Effective Cloud-based RAN Emulation System}
\author{}
\date{}
...
This artifact accompanies the paper \emph{EmuRAN: Scalable and Cost-Effective
Cloud-based RAN Emulation System}.  Its purpose is to make the implementation available for inspection,
provide the scripts used to deploy EmuRAN, provide the paper's plot outputs and
their underlying data, and demonstrate that the EmuRAN code is functional on a
...
view of EmuRAN operating end to end: its components start, coordinate RAN
...
\begin{itemize}
  ...
  hosted Powder deployment and verify EmuRAN's end-to-end operation.
...
The \code{code/} directory contains the implementation of EmuRAN:
...
\begin{itemize}
  \item \code{jupyter} and \code{IPython} for the notebook interface and
  inline display;
...
In the browser, select the Python kernel from the virtual environment.  Run an
individual cell to generate one figure, or select \emph{Run All} to execute
...
PDF and prints its path below the image.  A complete run produces 28 PDF files
...
The expected PDF count is \code{28}.  If a cell reports that it cannot find
...
The authors provide a running EmuRAN deployment on Powder for the artifact
...
\subsection{Step 2: inspect the running deployment}

The authors provide access to the already provisioned controller environment.
From that environment, check the cluster and deployed components:
...
The expected nodes should report \code{Ready}, and the provided EmuRAN pods
should report \code{Running}.  Topology changes should be made only through
the prescribed scale-reproduction workflow and access instructions supplied
through HotCRP.  If a component is not ready, report it through HotCRP so that
the authors can restore the shared deployment.

\subsection{Step 3: observe EmuRAN operation}

Inspect component placement and status:
...
Replace bracketed pod names with names reported by \code{kubectl}.  These are
read-only inspection commands.  The logs should show that the global
coordinator and proxy are advancing slots, the gNB is connected, the UE has
registered, and the UE data-plane container is exchanging traffic.

\subsection{Step 4: deployment success criteria}

The deployment checks pass when all of the following are observed:

\begin{itemize}
  \item Kubernetes reports the expected nodes as \code{Ready} and the EmuRAN
  pods as \code{Running};
  \item the global coordinator and proxy logs show ongoing slot coordination;
  \item the core/gNB logs show that the gNB connected;
  \item the UE log shows successful synchronization and registration; and
  \item the UE data-plane sidecar reports that the data plane is up and emits
  successful traffic measurements.
\end{itemize}

Success is determined by the operational checks above: component readiness,
slot coordination, UE registration, and working data-plane traffic.

\subsection{Step 5: reproduce scale-dependent dilation}
...
RAN slot eventually exceeds the available real-time compute budget.  EmuRAN
...
The authors provide several preconfigured scale points on the hosted Powder
deployment.  The exact scale values and the command that advances between
them are included with the SSH instructions in HotCRP so they can be matched
to the available Powder reservation.  At each scale, wait until the expected
gNB and UE pods are running and registered, then record recent global
coordinator samples:
...
The global coordinator reports one dilation sample for each 1,000 completed
slots.  The value is the wall-clock duration of those slots relative to their
nominal virtual-time duration.  A factor near 1 means the emulation is keeping
pace with real time; a factor above 1 means EmuRAN is deliberately allowing
additional wall-clock time so all components can complete each slot.

For each scale, compute the mean of the reported samples with:
...
Record the scale and mean dilation factor, then advance to the next supplied
scale point and repeat.  Use samples collected after all expected components
have registered, excluding deployment startup.  At least three increasing
scale points should be measured.
...
increase in dilation that enables EmuRAN to cope with insufficient compute at
higher scale while continuing coordinated slot execution.

\subsection{Step 6: finish the evaluation}

No cleanup or experiment termination is required from evaluators.  Log out of
the Powder host when inspection is complete.  The authors maintain the shared
deployment and will remove evaluator keys and terminate the experiment after
September 15.  Report failures, missing permissions, or questions through
HotCRP comments so that communication remains within the artifact-evaluation
process.
...
EmuRAN implementation executes end to end with working slot coordination,
...
as gNB/UE scale increases on fixed compute, EmuRAN increases its dilation
