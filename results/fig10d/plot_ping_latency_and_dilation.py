...
def collect_data(base_dir='.'):
    """Collect (latency, avg_ping_latency, std_ping_latency, avg_dilation) for all subdirs."""
    data = []
    latencies = ["0", "0.1", "0.2", "0.5", "1", "2", "5", "10"]
    for latency in latencies:
        ue_log = f"logs/ue-{latency}.log"
        globalsc_log = f"logs/globalsc-{latency}.log"
        if os.path.isfile(ue_log) and os.path.isfile(globalsc_log):
            ...
            if avg_ping is not None and avg_dilation is not None:
                data.append((latency, avg_ping, std_ping, avg_dilation))
    return data

def plot_dual_axis(data):
    latencies, pings, stds, dilations = zip(*data)
    fig, ax1 = plt.subplots(figsize=(3,2))

    color1 = 'tab:blue'
    ax1.set_xlabel('        Network Latencies (ms)', fontweight='bold', fontsize=13)
    ax1.set_yticks([])
    #ax1.set_ylabel('UE ping\nRTT (ms)', color=color1, fontweight='bold', fontsize=18)
    ax1.errorbar(latencies, pings, yerr=stds, marker='o', color=color1, label='UE Ping Latency', linestyle='-', capsize=4)
    ax1.tick_params(axis='y', labelcolor=color1, labelsize=16)
    ax1.tick_params(axis='x', labelsize=12)
    ax1.set_xticks(latencies, ["0", ".1", ".2", ".5", "1", "2", "5", "10"])
    for label in ax1.get_xticklabels():
        label.set_fontweight('bold')
    for label in ax1.get_yticklabels():
        label.set_fontweight('bold')
    #ax1.set_ylim(0, max(10, max([p+s for p,s in zip(pings,stds)])*1.1))
    ax1.set_ylim(0, 35)

    ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis

    color2 = 'tab:red'
    ax2.set_ylabel('Dilation\nFactor', color=color2, fontweight='bold', fontsize=18)
    ax2.plot(latencies, dilations, marker='s', color=color2, label='Dilation Factor', linestyle='--')
    ax2.tick_params(axis='y', labelcolor=color2, labelsize=16)
    for label in ax2.get_yticklabels():
        label.set_fontweight('bold')
    ax2.set_ylim(0, max(10, max(dilations)*1.1))
